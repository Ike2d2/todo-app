...
function renderList() {
  ...
  listItems.forEach((e, i) => {
    ...
    item.className = `flex h-20 w-full flex-shrink-0 px-3${
      i === selected ? " selected" : ""
    }`;
...
function renderMain() {
  ...
  toDisplay.items.forEach((e, i) => {
    ...
    task.className =
      "flex items-center justify-center h-full w-full flex-grow-1";
    ...
    const buttons = document.createElement("div");
    buttons.className =
      "buttons flex items-center justify-center text-3xl w-16";
